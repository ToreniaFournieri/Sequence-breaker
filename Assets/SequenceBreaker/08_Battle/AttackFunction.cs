...
sealed public class AttackFunction
{
    public AttackFunction(OrderClass order, List<BattleUnit> characters, EnvironmentInfoClass environmentInfo)
    {
        ...
        // no enemy anymore.
        var invalidAction = false || opponents.Count == 0;
        ...
        if (invalidAction == false)
        {
            ...
            var totalIndivisualHits = new int[characters.Count];
            ...
            var survivaledOpponents = opponents.FindAll(character1 => character1.Combat.HitPointCurrent > 0);
            ...
            for (var numberOfAttacks = 1; numberOfAttacks <= (int)(order.Actor.Combat.NumberOfAttacks * skillMagnificationNumberOfAttacks); numberOfAttacks++)
            {
                ...
                if (order.SkillEffectChosen.Skill.Magnification.AttackTarget == TargetType.multi)
                {
                    ...
                    if (isNeedCreateTargetPossibilityBox)
                    {
                        ...
                        var basicTargetRatio = 2.0 / 3.0;     //Tiar 1: Basic Target ratio
                        var optimumTargetRatio = 1.0 / 3.0;     //Tiar 2: Optinum Target ratio
                        double optimumTargetBonus = 0;
                        for (var opponent = 0; opponent < survivaledOpponents.Count; opponent++)
                        {
                ...
                if (toTarget != null && toTarget.Combat.HitPointCurrent > 0)
                {
                    ...
                    else //success!
                    {
                        ...
                        * environmentInfo.R.Next(40 + toTarget.Ability.Luck - criticalReduction, 100 - criticalReduction) / 100;
                        ...
                        var vsOffenseMagnification = 1.0;
                        ...
                        //vs Magnification offense
                        var vsDeffenseMagnification = 1.0;
                        ...
                        var dealtDamage = (int)((attackDamage) * damageTypeMagnification * vsOffenseMagnification * vsDeffenseMagnification
 ...
 " total hit" + snumberOfSuccessAttacks + ":" + numberOfSuccessAttacks + majorityElement + speedText + "\n";
            ...
            for (var fTargetColumn = 0; fTargetColumn <= opponents.Count - 1; fTargetColumn++)
            {
                ...
                if (totalIndivisualHits[opponents[fTargetColumn].UniqueID] >= 1)
                {
                    ...
                    var damageRateSpace = (4 - sign.Length - damageRatio.ToString().Length);
