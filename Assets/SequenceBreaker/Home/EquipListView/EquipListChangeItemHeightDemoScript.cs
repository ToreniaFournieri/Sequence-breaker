...
namespace SequenceBreaker.Home.HomeListView
{
    ...
    public class EquipListChangeItemHeightDemoScript : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            mLoopListView.InitListView(EquipListDataSourceMgr.instance.TotalItemCount, OnGetItemByIndex);
...
//        }
        ...
        {
            if (index < 0 || index >= EquipListDataSourceMgr.instance.TotalItemCount)
            {
                return null;
            }

            EquipListContentData itemData = EquipListDataSourceMgr.instance.GetItemDataByIndex(index);
            if (itemData == null)
            {
                return null;
            }
            LoopListViewItem2 item = listView.NewListViewItem("ItemPrefab1");
            EquipListItem itemScript = item.GetComponent<EquipListItem>();
        ...
        void OnAddItemBtnClicked()
        {
            ...
            if (count < 0 || count > EquipListDataSourceMgr.instance.TotalItemCount)
            {
                return;
            }
