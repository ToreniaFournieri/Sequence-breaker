    ...
    public class EnhancedScroller : MonoBehaviour
    {
            ...
            )
        {
            ...
            if (loop)
            {
                ...
                var set1Position = GetScrollPositionForCellViewIndex(_loopFirstCellIndex - (NumberOfCells - dataIndex), CellViewPositionEnum.Before) + offset;
                var set2Position = GetScrollPositionForCellViewIndex(_loopFirstCellIndex + dataIndex, CellViewPositionEnum.Before) + offset;
                var set3Position = GetScrollPositionForCellViewIndex(_loopFirstCellIndex + NumberOfCells + dataIndex, CellViewPositionEnum.Before) + offset;
        ...
        private int _GetCellIndexAtPosition(float position, int startIndex, int endIndex)
        {
            ...
            if ((_cellViewOffsetArray[middleIndex] + (scrollDirection == ScrollDirectionEnum.Vertical ? padding.top : padding.left)) >= position + 1.00001f)
